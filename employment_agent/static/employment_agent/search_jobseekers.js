...
function updateTableAndControls(result) {
    ...
    let tableRef = document.querySelector('#jobseekers-table').getElementsByTagName('tbody')[0]
    ...
    for (const employer of data) {
        ...
        //now the date
        let dateString = employer.fields.date_applied;
        ...
        if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
            ...
            row.innerHTML = `<td><strong>Firstname:</strong> ${employer.fields.firstname}</td>` +
                ...
                `<td><strong>Date applied:</strong> ${date.toDateString()}</td>` +
                `<td id="controls">` +
...
function getEmployerData(page) {
    $.ajax({
        url: `http://localhost:8000/job_seekers/search/${page}/`,
        type: 'GET',
...
document.addEventListener('DOMContentLoaded', (e) => {
    ...
    searchForm.addEventListener('submit', (event) => {
        ...
        $.ajax({
            url: 'http://localhost:8000/job_seekers/search/1/',
            type: 'POST',
